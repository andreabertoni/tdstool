...
SUBROUTINE write_3D_cplx_in_file_gnuplot(path, fname, iter, psi, xnodes, ynodes, znodes, numx, numy, numz, downsample_x, downsample_y, downsample_z)
  CHARACTER(*), INTENT(IN) :: path, fname
  INTEGER, INTENT(IN) :: iter, numx, numy, numz, downsample_x, downsample_y, downsample_z
  COMPLEX*16, INTENT(IN) :: psi(:)
  REAL*8 xnodes(:), ynodes(:), znodes(:)
  ...
  DO nx = 1, numx, downsample_x
    DO ny = 1, numy, downsample_y
        DO nz = 1, numz, downsample_z
            WRITE (10, *) xnodes(nx+1), ynodes(ny+1), znodes(nz+1), abs(psi((nx-1)*numy*numz + (ny-1)*numz + nz))**2
        END DO
    END DO
  END DO
  close(10)
...
SUBROUTINE write_3D_real_in_file_gnuplot(path, fname, iter, psi, xnodes, ynodes, znodes, numx, numy, numz, downsample_x, downsample_y, downsample_z)
  CHARACTER(*), INTENT(IN) :: path, fname
  INTEGER, INTENT(IN) :: iter, numx, numy, numz, downsample_x, downsample_y, downsample_z
  REAL*8, INTENT(IN) :: psi(:,:,:)
  REAL*8 xnodes(:), ynodes(:), znodes(:)
  ...
  DO nx = 1, numx, downsample_x
    DO ny = 1, numy, downsample_y
        DO nz = 1, numz, downsample_z
            WRITE (10, *) xnodes(nx+1), ynodes(ny+1), znodes(nz+1), psi(nz, ny, nx)
        END DO
    END DO
  END DO
  close(10)
...
SUBROUTINE write_3D_cplx_in_file_bin(path, fname, iter, psi, numx, numy, numz, downsample_x, downsample_y, downsample_z)
  CHARACTER(*), INTENT(IN) :: path, fname
  INTEGER, INTENT(IN) :: iter, numx, numy, numz, downsample_x, downsample_y, downsample_z
  COMPLEX*16, INTENT(IN) :: psi(:)
  COMPLEX*16, ALLOCATABLE :: psi_down(:)
  ...
  CHARACTER(4) :: filenum
  INTEGER nx, ny, nz, counter

  filenum = STRING(iter)
  open(30, FILE=TRIM(path)//"/"//fname//filenum//".bin", FORM="UNFORMATTED", STATUS='REPLACE')

  if (downsample_x == 1 .and. downsample_y == 1 .and. downsample_z == 1) then
    WRITE (30) psi
  else
    ALLOCATE (psi_down((numx/downsample_x)*(numy/downsample_y)*(numz/downsample_z)))
    counter = 1;
    do nx = 1,numx,downsample_x
      do ny = 1,numy,downsample_y
        do nz = 1,numz,downsample_z
          psi_down(counter) = psi((nx-1)*numy*numz + (ny-1)*numz + nz)
          counter = counter+1
        end do
      end do
    end do
...
SUBROUTINE write_3D_real_in_file_bin(path, fname, iter, psi, numx, numy, numz, downsample_x, downsample_y, downsample_z)
  CHARACTER(*), INTENT(IN) :: path, fname
  INTEGER, INTENT(IN) :: iter, numx, numy, numz, downsample_x, downsample_y, downsample_z
  REAL*8, INTENT(IN) :: psi(:,:,:)
  REAL*8, ALLOCATABLE :: psi_down(:)
...
SUBROUTINE write_3D_grid(path, fname, xnodes, ynodes, znodes, numx, numy, numz, downsample_x, downsample_y, downsample_z)
  CHARACTER(*), INTENT(IN) :: path, fname
