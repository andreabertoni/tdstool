...
!************************************************************************
SUBROUTINE strtopot3d( npotloop, mstar, strpotential, numpx, numpy, numpz, px, py, pz, pot )
  IMPLICIT NONE
! Creates the 2d structure potential pot (in eV), reading values in
! eV from the definition string strpotential
!
! keywords: 
! box      xLowerLeft  yLowerLeft  zLowerLeft  xUpperRight  yUpperRight  zUpperRight  val(eV)
!
! each argument "loop" is substituted with 
!       potloopvar_from + (potloopvar_step * npotloop)
!
...
INTEGER, INTENT(IN) :: npotloop     ! n. of cycle loop on potential
CHARACTER(999), INTENT(INOUT) :: strpotential
INTEGER, INTENT(IN) :: numpx, numpy, numpz
REAL*8, INTENT(IN) :: mstar
REAL*8, INTENT(IN) :: px(1:numpx), py(1:numpy), pz(1:numpz)
REAL*8, INTENT(OUT) :: pot(1:numpz, 1:numpy, 1:numpx)  ! the potential

CHARACTER(200) :: strcommand, strkeyword, strvalues
INTEGER :: lenstr
REAL*8 :: potdefvals(10)
!REAL*8 :: paux
INTEGER :: npx, npy, npz, nn, nnp
...
DO nn= 1, lenstr
  IF ( strpotential(nn:nn)==";" ) THEN
    ...
    SELECT CASE (strkeyword)
    CASE ("box", "BOX")
      CALL strtopot_valsread( npotloop, strvalues, 7, potdefvals(:) )
      DO npx= 1, numpx
        IF (px(npx)>=potdefvals(1) .AND. px(npx)<=potdefvals(4)) THEN
          DO npy= 1, numpy
            IF (py(npy)>=potdefvals(2) .AND. py(npy)<=potdefvals(5)) THEN
              DO npz= 1, numpz
                IF (pz(npz)>=potdefvals(3) .AND. pz(npz)<=potdefvals(6)) THEN
                  pot(npz,npy,npx)= potdefvals(7)
                END IF
              END DO
            END IF
          END DO
